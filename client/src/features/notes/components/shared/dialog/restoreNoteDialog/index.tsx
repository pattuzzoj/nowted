...
interface RestoreNoteDialogProps extends ParentProps {
  note: Note;
}

export default function RestoreNoteDialog(props: RestoreNoteDialogProps) {
        ...
        <Dialog.Content>
          <div class="absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 z-10 w-11/12 max-w-96 flex flex-col gap-6 p-4 rounded-lg bg-layout-primary border-2 border-white/10">
            <div class="flex items-center justify-between">
              <h4 class="font-medium">Restore Note</h4>
              <Dialog.CloseTrigger title="close form" class="dialog-btn dialog-btn-close">
                <CloseIcon />
              </Dialog.CloseTrigger>
            </div>
            <div class="flex flex-col gap-6">
              <Select.Root collection={collection()} positioning={{ sameWidth: true }} onValueChange={({ value }) => setFolderId(value[0])}>
                <Select.Control>
              ...
              <div class="flex flex-col gap-2">
                <RestoreNoteAction class="justify-center bg-primary" note={props.note} folderId={folderId()}>
                  Restore
                </RestoreNoteAction>
