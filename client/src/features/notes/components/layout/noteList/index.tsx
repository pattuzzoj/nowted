...
import type { Note as INote } from "@/features/notes/types";
...
export default function NoteList(props: NoteListProps) {
  ...
  let contextHref = () => (props.context === "folder") ? `/folder/${data.folder.id}/note` : `/${props.context}`;
  ...
  return (
    <section class="md:min-w-64 h-full md:h-screen w-full flex flex-col gap-5 p-2 bg-layout-secondary">
      <header class="flex justify-between items-center">
        ...
        <Show when={props.context === "folder"}>
          <CreateNoteDialog>
      ...
      <Show when={props.list.length > 0} fallback={props.fallback}>
