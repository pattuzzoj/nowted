...
export default function MediaMenu(props: MarkProps) {
  ...
  return (
    <span class="flex gap-1">
      <button title="image" class="flex items-center btn" onClick={handlerSetImage}>
        <ImagePlus class="size-5" />
      </button>
      <label title="image" class="flex items-center btn cursor-pointer">
        <input class="hidden" type="file" accept="image/*" multiple onChange={handlerUploadImage} />
        <ImageUp class="size-5" />
      </label>
      <button title="link" class="flex items-center btn" onClick={handlerSetLink}>
        <Link class="size-5" />
      </button>
    </span>
  )
