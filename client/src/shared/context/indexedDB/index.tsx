import { createContext, createSignal, JSXElement, onCleanup, onMount, Show } from "solid-js";
...
export const IndexedDBContext = createContext<IndexedDBContextType>([] as any);
...
export interface StoreOperations<T> {
  ...
  openCursor(callback: (cursor: IDBCursorWithValue) => T, {query, direction, index}?: {query?: Key, direction?: IDBCursorDirection, index?: string}): Promise<T[]>;
  openKeyCursor(callback: (cursor: IDBCursor) => T, {query, direction, index}?: {query?: Key, direction?: IDBCursorDirection, index?: string}): Promise<T[]>;
...
export default function IndexedDBProvider(props: IndexedDBProps) {
  ...
  function useStore<T>(name: string): StoreOperations<T> {
    ...
    function openCursor(callback: (cursor: IDBCursorWithValue) => T, {query, direction, index}: {query?: Key, direction?: IDBCursorDirection, index?: string} = {}): Promise<T[]> {
      return new Promise(async (resolve, reject) => {
        try {
          const store = (await transaction(name, "readonly")).objectStore(name);
          let storeRequest: IDBRequest<IDBCursorWithValue | null>;

          if (index) {
            storeRequest = store.index(index).openCursor(query, direction);
          } else {
            storeRequest = store.openCursor(query, direction);
          }
    ...
    function openKeyCursor(callback: (cursor: IDBCursor) => T, {query, direction, index}: {query?: Key, direction?: IDBCursorDirection, index?: string} = {}): Promise<T[]> {
      return new Promise(async (resolve, reject) => {
        try {
          const store = (await transaction(name, "readonly")).objectStore(name);
          let storeRequest: IDBRequest<IDBCursor | null>;

          if (index) {
            storeRequest = store.index(index).openKeyCursor(query, direction);
          } else {
            storeRequest = store.openKeyCursor(query, direction);
          }
