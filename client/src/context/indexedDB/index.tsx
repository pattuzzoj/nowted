...
export default function IndexedDBProvider(props: IndexedDBProps) {
  ...
  function transaction(name: string, mode: Exclude<IDBTransactionMode, "versionchange">): IDBTransaction | undefined {
  ...
  function useStore<T>(name: string): StoreOperations<T> {
    ...
    function add(data: T, key?: Exclude<Key, IDBKeyRange>): Promise<Key> {
      return new Promise((resolve, reject) => {
        try {
          const store = transaction(name, "readwrite")?.objectStore(name); 
    ...
    function get(key: Key): Promise<T> {
      return new Promise((resolve, reject) => {
        try {
          const store = transaction(name, "readonly")?.objectStore(name); 
